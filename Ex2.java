/*
 * I've decided to use ArrayLists because I already used them in a method and I am the most
 * comfortable with them for this. Because the robotdata doesn't have a max as before which
 * represented the size of every array used in the robotdata, my implementation already saves a lot
 * of space. I still use the location and heading for junctions but being an arraylist, it only has
 * the dimension of how many elements are needed. To further efficientize the space, after the
 * robot backs up through a junction, it gets deleted because it won't ever pass that one ever.
 *
 * Improvement was made with the majority of methods implemented in the robotdata class. The choice
 * of arraylist instead of classic array and saving only the needed information and getting rid of
 * the elements not needed anymore is the right choice. Therefore it enables the removal of for
 * loops that were iterating through the entire arrays to check and add junctionrecorder elements
 * (heading, and coordinates). Now they are only one step, thus improving a lot the time efficiency
 * of the code.
 *
